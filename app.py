...
def pdf_nova_pagina(c, W: float, H: float, margem: float, y: float, min_y: float):
    ...
    c.setLineWidth(0.35)
    c.line(margem, H - 0.9 * cm, W - margem, H - 0.9 * cm)
    return H - margem - 1.15 * cm
# ==================================================
# CABEÇALHO
# ==================================================
...
if enviar:
    ...
    W, H = A4
    margem = 2 * cm
    min_y_conteudo = 2.6 * cm
    # ReportLab: usar ImageReader(BytesIO(...)) — BytesIO puro em drawImage gera TypeError no Cloud.
    ...
    c.setFillColor(colors.white)
    c.rect(0, 0, W, H, fill=1, stroke=0)
    # ── Cabeçalho: logo no topo, textos sempre ABAIXO do logo (sem sobreposição) ──
    # Em ReportLab, y cresce para cima; topo da página = H. drawImage usa canto inferior esquerdo.
    largura_logo = 4.2 * cm
    altura_logo = largura_logo * (ih / iw)
    margem_topo = 1.0 * cm
    logo_topo_y = H - margem_topo
    y_logo_bottom = logo_topo_y - altura_logo
    ...
    gap_abaixo_logo = 0.78 * cm
    titulo_y = y_logo_bottom - gap_abaixo_logo
    c.setFillColor(colors.HexColor(PRIMARY))
    c.setFont("Helvetica-Bold", 14)
    c.drawCentredString(W / 2, titulo_y, "FORMULÁRIO DE JUSTIFICATIVA DE PONTO")
    subtitulo_y = titulo_y - 0.58 * cm
    c.setFont("Helvetica", 10)
    c.setFillColor(colors.HexColor(MUTED))
    c.drawCentredString(W / 2, subtitulo_y, "Hospital Regional Sul")
    c.setFillColor(colors.black)
    y_linha_accent = subtitulo_y - 0.52 * cm
    c.setStrokeColor(colors.HexColor(ACCENT))
    c.setLineWidth(1.05)
    c.line(margem, y_linha_accent, W - margem, y_linha_accent)
    y = y_linha_accent - 0.62 * cm
    c.setStrokeColor(colors.HexColor(BORDER))
    c.setLineWidth(0.45)
    ...
    # ── Bloco: Dados do plantão (tabela clara, fundo branco) ──
    y -= 1.1 * cm
    ...
    vx = margem + label_w
    line_extra = 0.36 * cm
    inner_left = margem + 0.5 * cm
    for i, (titulo, valor) in enumerate(campos_pdf):
        linhas_val = quebrar_texto(str(valor), limite=70)
        row_h = max(
            0.78 * cm,
            0.48 * cm + max(0, len(linhas_val) - 1) * line_extra + 0.2 * cm,
        )
        y = pdf_nova_pagina(c, W, H, margem, y, min_y_conteudo + row_h + 0.35 * cm)
        c.setStrokeColor(colors.HexColor(BORDER))
        c.setLineWidth(0.35)
        c.line(margem, y - row_h, W - margem, y - row_h)
        c.setFont("Helvetica-Bold", 9.5)
        c.setFillColor(colors.HexColor("#334155"))
        c.drawString(inner_left, y - 0.48 * cm, titulo)
        c.setFont("Helvetica", 10)
        c.setFillColor(colors.black)
        for j, lv in enumerate(linhas_val):
            c.drawString(vx, y - 0.48 * cm - j * line_extra, lv)
        y -= row_h
    # ── Justificativa ──
    y -= 1.0 * cm
    ...
    y -= 0.55 * cm
    linhas = quebrar_texto(motivo.strip(), limite=88)
    line_height = 13
    pad = 16
    altura_box = len(linhas) * line_height + pad
    y = pdf_nova_pagina(c, W, H, margem, y, min_y_conteudo + altura_box * 0.45)
    c.setFillColor(colors.white)
    c.setStrokeColor(colors.HexColor(BORDER))
    c.setLineWidth(0.55)
    c.roundRect(margem, y - altura_box, W - 2 * margem, altura_box, 5, stroke=1, fill=1)
    texto = c.beginText(margem + 12, y - 22)
    texto.setFont("Helvetica", 10)
    texto.setLeading(line_height)
    texto.setFillColor(colors.HexColor("#0f172a"))
    for linha in linhas:
        texto.textLine(linha)
    c.drawText(texto)
    # ── Assinatura ──
    y -= altura_box + 2.2 * cm
    y = pdf_nova_pagina(c, W, H, margem, y, min_y_conteudo + 3.4 * cm)
    sig_h = 3.5 * cm
    sig_left = margem
    sig_w = W - 2 * margem
    c.setFillColor(colors.white)
    c.setStrokeColor(colors.HexColor(BORDER))
    c.setLineWidth(0.55)
    c.roundRect(sig_left, y - sig_h, sig_w, sig_h, 6, stroke=1, fill=1)
    accent_x = sig_left + 0.4 * cm
    c.setFillColor(colors.HexColor(ACCENT))
    c.rect(accent_x, y - sig_h + 0.35 * cm, 0.1 * cm, sig_h - 0.75 * cm, fill=1, stroke=0)
    c.setFillColor(colors.black)
    tx = sig_left + 0.62 * cm
    c.setFont("Helvetica-Bold", 10)
    c.setFillColor(colors.HexColor(PRIMARY))
    c.drawString(tx, y - 0.5 * cm, "Assinatura do médico")
    c.setFont("Helvetica-Oblique", 8)
    c.setFillColor(colors.HexColor(MUTED))
    c.drawString(tx, y - 0.95 * cm, "Nome completo conforme registro no CRM")
    c.setFillColor(colors.black)
    linha_y = y - 2.5 * cm
    line_left = tx
    line_right = sig_left + sig_w - 0.5 * cm
    c.setStrokeColor(colors.HexColor(ACCENT))
    c.setLineWidth(0.9)
    c.line(line_left, linha_y, line_right, linha_y)
    c.setStrokeColor(colors.HexColor(BORDER))
    c.setLineWidth(0.35)
    c.line(line_left, linha_y - 0.1 * cm, line_right, linha_y - 0.1 * cm)
    c.setFont("Helvetica-Bold", 10.5)
    c.setFillColor(colors.HexColor(PRIMARY))
    c.drawString(line_left, linha_y + 0.24 * cm, assinatura)
    c.setFont("Helvetica", 7.5)
    c.setFillColor(colors.HexColor(MUTED))
    c.drawString(line_left, linha_y - 0.38 * cm, "Assinatura ou carimbo quando aplicável")
    c.setFont("Helvetica-Oblique", 8)
    c.setFillColor(colors.HexColor(MUTED))
    c.drawCentredString(
        W / 2,
