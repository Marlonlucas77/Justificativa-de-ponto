...
if enviar:
    ...
    # ── Cabeçalho (linhas discretas + logo; sem faixa colorida) ──
    c.setStrokeColor(colors.HexColor(BORDER))
    c.setLineWidth(0.6)
    c.line(margem, H - 1.0 * cm, W - margem, H - 1.0 * cm)

    largura_logo = 4.4 * cm
    altura_logo = largura_logo * (ih / iw)
    x_logo = (W - largura_logo) / 2
    y_logo_bottom = H - 1.35 * cm - altura_logo
    c.drawImage(
    ...
    c.setLineWidth(1.0)
    c.line(margem, y_logo_bottom - 0.45 * cm, W - margem, y_logo_bottom - 0.45 * cm)

    y = y_logo_bottom - 1.05 * cm
